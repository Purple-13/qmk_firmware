...
// 										//qmk compile -kb handwired/purple13/split_astar_micro -km v10

																			//Definitions  
#define CUT RCTL(KC_X)					// cut
...
#define CHAT_OPN TD(TD_CHAT_OPN)		// sent enter and swap to a dvorak layout
#define CHAT_CLS TD(TD_CHAT_CLS)		// sent enter and swap to gaming layout

#define SYM_SPC LT(_SYMBOLS,KC_SPC)		// Tap once for space, hold for symbol layer
#define NUM_ENT LT(_NAV_NUM,KC_ENT)		// Tap once for enter, hold for num layer
...
* 	|-----------------------------------------------------------------------------------|           		|-----------------------------------------------------------------------------------|
*  				 			 			|	 ALT	|	CTRL	|	SHIFT	|	SPACE	|		\**\		| CHAT_OPN	|	  		|	F20		|	 F21	|	
*       							  	`-----------------------------------------------' 					`-----------------------------------------------'    
*/
[_GAMING] = LAYOUT(
		...
		KC_END, 	KC_B, 		KC_Z, 		KC_X, 		KC_C, 		KC_V, 		KC_F5,			/**/			KC_NO,		KC_NO, 		KC_NO, 		KC_NO, 		KC_NO, 		KC_NO, 		KC_NO,
											KC_LALT,	KC_LCTL,	KC_LSFT, 	KC_SPC,       	/**/			CHAT_OPN, 	KC_NO, 		KC_F20, 	KC_F21							
),
/* 																							   DVORAK		
*	 ,----------------------------------------------------------------------------------.           		,-----------------------------------------------------------------------------------. 
...
* 	|    TAB	|	 ;:		|	  Q		|	  J		|	  K		|	  X		|	 =/		|       \**\		|	 ?\		|	 B		|	 M		|	 W		|	 V		|	 Z		|    TAB	|
* 	|-----------------------------------------------------------------------------------|           		|-----------------------------------------------------------------------------------|
*  				 			 			|	SHIFT	|	 CTRL	|  CHAT_CLS	|	SPC		|		\**\		|  CHAT_CLS	|	SPC		|	ALT		|	SHIFT	|	
*       							  	`-----------------------------------------------' 					`-----------------------------------------------'            
*/
[_CHAT] = LAYOUT(
		...
		KC_DEL,		KC_A, 		KC_O, 		KC_E, 		KC_U, 		KC_I, 		LBRAC,			/**/			RBRAC,		KC_D, 		KC_H, 		KC_T, 		KC_N, 		KC_S, 		KC_BSPC,
		KC_TAB, 	KC_SCLN, 	KC_Q, 		KC_J, 		KC_K, 		KC_X, 		LEQL,			/**/			RQUES,		KC_B, 		KC_M, 		KC_W, 		KC_V, 		KC_Z, 		KC_TAB,
											KC_LSFT,	KC_LCTL, 	CHAT_CLS, 	KC_SPC, 		/**/			CHAT_CLS, 	KC_SPC, 	KC_RALT, 	KC_RSFT
), 	
/*																								NUM	
*	 ,----------------------------------------------------------------------------------.           		,-----------------------------------------------------------------------------------. 
* 	|	 	 	|			| 			|	  Up	|			|	Cut		|	GAMING	|       \**\ 		|	DVORAK	|			|	  7		|	  8		|	  9		|			| 			|
* 	|-----------------------------------------------------------------------------------|           		|-----------------------------------------------------------------------------------|
* 	|	 DEL 	|			|	 Left 	|	 Down	|	Right	|	Copy	|	 [<		|		\**\		|	  ]>	|	  .		|	  4		|	  5		|	  6		|	  0		| BACKSPACE |
* 	|-----------------------------------------------------------------------------------|           		|-----------------------------------------------------------------------------------|
* 	|			|			|			|			|			|	UNDO	|	 +-		|       \**\		|	  _|	|			|	  1		|	  2		|	  3		|			|			|
* 	|-----------------------------------------------------------------------------------|           		|-----------------------------------------------------------------------------------|
*  				 			 			|	SHIFT	|	 CTRL	|	TRNS	|	SPACE	|		\**\		|	TRNS	|	SPACE	|	ALT		|	SHIFT	|	
*       							  	`-----------------------------------------------' 					`-----------------------------------------------'    
*/	
[_NAV_NUM] = LAYOUT(
																																															
		KC_NO,		KC_NO,	 	KC_NO,		KC_UP, 		KC_NO,		CUT, 		GAMING,			/**/			DVORAK,		KC_NO, 		KC_7, 		KC_8, 		KC_9,		KC_NO, 		KC_NO,
		KC_DEL,	 	KC_NO,		KC_LEFT, 	KC_DOWN,  	KC_RIGHT, 	CPYPST, 	LSBRAC,			/**/			RSBRAC,		KC_DOT, 	KC_4, 		KC_5, 		KC_6, 		KC_0, 		KC_BSPC,
		KC_NO, 		KC_NO,	 	KC_NO,		KC_NO, 		KC_NO, 		UNDO, 		LPLUS,			/**/			RUNDS,		KC_NO, 		KC_1, 		KC_2, 		KC_3, 		KC_NO, 		KC_NO,
											KC_LSFT,	KC_LCTL, 	KC_TRNS, 	KC_SPC, 		/**/			KC_TRNS, 	KC_SPC, 	KC_RALT, 	KC_LSFT
),
/* 																								SYM			
*	 ,----------------------------------------------------------------------------------.           		,-----------------------------------------------------------------------------------. 
...
* 	|			|	  		|	  		|	 		|	  		|			|	 +-		|       \**\		|	  _|	|			|	 F9		|	 F10	|	 F11	|	 F12	|	   		|
* 	|-----------------------------------------------------------------------------------|           		|-----------------------------------------------------------------------------------|
*  				 			 			|	SHIFT	|	CTRL	|	ENT		|	TRNS	|		\**\		|	ENT		|	TRNS	|	 ALT	|	SHIFT	|	
*       							  	`-----------------------------------------------' 					`-----------------------------------------------'    
*/
[_SYMBOLS] = LAYOUT(
		...
		KC_NO, 		KC_NO, 		KC_NO, 		KC_NO, 		KC_NO, 		KC_NO, 		LPLUS,			/**/			RUNDS, 		KC_NO,	 	KC_F9, 		KC_F10, 	KC_F11, 	KC_F12, 	KC_NO,
											KC_LSFT,	KC_LCTL, 	KC_ENT,		KC_TRNS, 		/**/			KC_ENT,		KC_TRNS, 	KC_RALT, 	KC_RSFT
)
};
...
// For the TD_CHAT_CLS tap dance. 
void TD_CHAT_CLS_finished(qk_tap_dance_state_t *state, void *user_data);
void TD_CHAT_CLS_reset(qk_tap_dance_state_t *state, void *user_data);

// For the TD_CHAT_OPN tap dance. 
void TD_CHAT_OPN_finished(qk_tap_dance_state_t *state, void *user_data);
void TD_CHAT_OPN_reset(qk_tap_dance_state_t *state, void *user_data);
...
qk_tap_dance_action_t tap_dance_actions[] = {   							//Registers all tap dances
    ...
    [TD_CHAT_CLS] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, TD_CHAT_CLS_finished, TD_CHAT_CLS_reset),		// Tap once for enter, hold for num layer
	
    [TD_CHAT_OPN] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, TD_CHAT_OPN_finished, TD_CHAT_OPN_reset),	// Tap once for chat, hold for num layer
	...
	[TD_UNDS_PIPE] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, TD_UNDS_PIPE_finished, TD_UNDS_PIPE_reset),	// Tap for _, hold for |
																			...
																			//TD_CHAT_OPN tap dance  		--send enter and swap to a dvorak layout
static td_tap_t TD_CHAT_OPN_tap_state = {
    .is_press_action = true,
    .state = TD_NONE
};

void TD_CHAT_OPN_finished(qk_tap_dance_state_t *state, void *user_data) {
   TD_CHAT_OPN_tap_state.state = cur_dance(state);
    switch (TD_CHAT_OPN_tap_state.state) {
...
void TD_CHAT_OPN_reset(qk_tap_dance_state_t *state, void *user_data) {
    ...
    TD_CHAT_OPN_tap_state.state = TD_NONE;
};


																			//TD_CHAT_CLS tap dance     	--send enter and swap to gaming layout																		
static td_tap_t TD_CHAT_CLS_tap_state = {
    .is_press_action = true,
    .state = TD_NONE
};

void TD_CHAT_CLS_finished(qk_tap_dance_state_t *state, void *user_data) {
    TD_CHAT_CLS_tap_state.state = cur_dance(state);
    switch (TD_CHAT_CLS_tap_state.state) {
        case TD_SINGLE_TAP: break;
...
void TD_CHAT_CLS_reset(qk_tap_dance_state_t *state, void *user_data) {
    ...
    TD_CHAT_CLS_tap_state.state = TD_NONE;
};
